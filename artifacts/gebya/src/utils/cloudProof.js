...
export const LOCAL_SYNC_STATUS = 'local_only';
...
export async function createCloudProofFields() {
  ...
  const createdAtDevice = Date.now();
  return {
    device_id: deviceId,
...
export function buildCloudProofPayload({ recordTable, recordType, record }) {
  const base = {
    ...
    created_at_device: timestampOrNull(record.created_at_device),
    updated_at: timestampOrNull(record.updated_at),
    actor_staff_member_id: actorStaffReference(record),
    sync_status: record.sync_status || LOCAL_SYNC_STATUS,
