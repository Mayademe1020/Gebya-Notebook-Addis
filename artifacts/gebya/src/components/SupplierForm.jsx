      ...
      <div className="bg-white w-full max-w-md max-h-[92vh] overflow-y-auto animate-slide-up" style={{ borderRadius: 'var(--radius-xl) var(--radius-xl) 0 0', boxShadow: 'var(--shadow-lg)' }}>
        <div className="sticky top-0 bg-white z-10 px-6 pt-5 pb-4 border-b" style={{ borderRadius: 'var(--radius-xl) var(--radius-xl) 0 0', borderColor: 'var(--color-border-light)' }}>
          <div className="flex justify-between items-center gap-3">
            <div>
              <h2 className="text-xl font-bold text-gray-900">{t.addSupplier || 'Add supplier'}</h2>
            </div>
            <button onClick={onDone} aria-label={t.close || 'Close'} className="p-2 rounded-full hover:bg-gray-100 transition-colors min-w-[44px] min-h-[44px] flex items-center justify-center press-scale">
        ...
        <div className="px-6 py-4 space-y-4">
          <div>
            <label className="block text-gray-700 font-semibold mb-2">
              {t.supplierName || 'Supplier name'} <span className="text-red-500">*</span>
            </label>
            <input
              type="text"
              value={displayName}
              onChange={(e) => setDisplayName(e.target.value)}
              placeholder={t.supplierNamePlaceholder || 'Supplier name'}
              autoFocus
          ...
          <div>
            <div className="flex items-center justify-between gap-3 mb-2">
              ...
              <button
                ...
                className="px-3 py-2 text-xs font-bold border min-h-[40px] press-scale disabled:opacity-45 disabled:cursor-not-allowed"
              ...
              >
                {t.pickFromContacts || 'Pick from contacts'}
              </button>
            </div>
            ...
            {phoneTouched && phoneEntered && !phoneValid && (
              <p className="text-xs mt-1.5 font-medium text-red-600">
                {t.phoneMustStartWith7or9 || 'Phone must start with 7 or 9 (9 digits)'}
              </p>
            )}
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              placeholder={t.supplierNotePlaceholder || 'Note'}
              rows={3}
        ...
        <div className="px-6 pb-8 pt-2">
          <button onClick={handleSave} disabled={!canSave || saving} className="w-full p-4 font-bold text-white text-base flex items-center justify-center gap-2 min-h-[56px] press-scale" style={{ background: canSave ? '#1B4332' : '#e5e7eb', color: canSave ? '#fff' : '#9ca3af', borderRadius: 'var(--radius-md)', boxShadow: canSave ? '0 4px 0 #0f2b20, var(--shadow-sm)' : 'none' }}>
            ...
            {saving ? (t.saving || 'Saving...') : (t.saveSupplier || 'Save supplier')}
          </button>
        </div>
      </div>
    </div>
